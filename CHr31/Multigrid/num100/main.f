...
C ... input and output files ...
        ...
        open(unit=6,file='mg.out',status='new')
...
C ... (2,1) V or W cycles ...
 ...
 25     if(ic(capm).gt.0) then
          if((ic(k).eq.0) .or. (k.eq.1)) then
            ...
            k = k+1
            call relax(k,u,f)
            if(k.eq.capm) call err(k,u,ex,f)
            ic(k) = ic(k) - 1
          else
            call relax(k,u,f)
            call relax(k,u,f)
            call resinj(k,k-1,u,f)
            call putzero(k-1,u)
            k = k-1
            ic(k) = gamma
          endif
...
c ... subroutines ...
        subroutine resinj(k,kc,v,f)

        implicit none

C ... variable declarations ...

        integer i,j,if,jf,imax,imaxc,jmax,jmaxc,k,kc

        include 'incs.com'

        integer p(0:npmax),pc(0:npmax)
        real h,hc,t,v(0:gptmax),f(0:gptmax)

C ... specifiers for grid kc ...

        call key(k,p,imax,jmax,h)
        call key(kc,pc,imaxc,jmaxc,hc)

C ... compute residual and inject to coarser grid ...
        ...
        do 10 i=1,imaxc
          ...
          do 10 j=1,jmaxc
            jf = 2*j
            t = f(p(if)+jf) + v(p(if+1)+jf) + v(p(if-1)+jf)
     &          + v(p(if)+jf+1) + v(p(if)+jf-1) - 4.*v(p(if)+jf)
            f(pc(i)+j) = 2.*t 
 10     continue
